...
import { FaCheckCircle, FaBoxes, FaStore } from 'react-icons/fa';
...
const RetailerSideNav: React.FC<{isExpanded: boolean, toggleSidebar: () => void}> = ({ isExpanded, toggleSidebar }) => {
  ...
  const navItems: NavItem[] = [
    ...
    { to: "marketplace", icon: <FaStore />, name: "Marketplace" }, // ✅ Added Marketplace
  ];
  ...
  const NavItemComponent: React.FC<{ to: string, icon: React.ReactNode, name: string }> = ({ to, icon, name }) => (
    <NavLink
      ...
      className={({ isActive }) =>
        ...
        } ${isActive ? 'bg-white text-black' : 'text-gray-400 hover:bg-gray-700 hover:text-white'}`
      }
    >
  ...
  return (
    <aside
      className={`fixed left-0 top-0 z-20 flex h-screen flex-col bg-[#1c2536] text-white transition-all duration-300 ease-in-out ${
    ...
    >
      <div className="relative flex h-16 items-center justify-center border-b border-gray-800 p-4">
        ...
        <button
          ...
          className="absolute right-[-12px] top-1/2 z-20 -translate-y-1/2 rounded-full bg-gray-700 p-0.5 text-2xl text-white"
        >
      ...
      <div className="flex flex-1 flex-col overflow-y-auto p-2">
        ...
        <div className="mt-auto border-t border-gray-700 pt-2">
          ...
          <button
            ...
            className={`flex w-full items-center p-3 my-1 rounded-lg text-gray-400 transition-colors hover:bg-red-500 hover:text-white ${
              isExpanded ? 'justify-start' : 'justify-center'
            }`}
          >
