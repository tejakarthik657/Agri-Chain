...
import { FaTruck, FaHistory } from 'react-icons/fa';
...
const LogisticsSideNav: React.FC<{isExpanded: boolean, toggleSidebar: () => void}> = ({ isExpanded, toggleSidebar }) => {
  const navigate = useNavigate();
  const handleLogout = () => navigate('/');

  const navItems: NavItem[] = [
    { to: "dashboard", icon: <BsGrid1X2Fill />, name: "Dashboard" },
    { to: "update-status", icon: <FaTruck />, name: "Update Status" },
    { to: "shipment-history", icon: <FaHistory />, name: "Shipment History" },
  ];
  ...
  return (
    <aside className={`fixed left-0 top-0 z-20 flex h-screen flex-col bg-[#1c2536] text-white transition-all duration-300 ease-in-out ${isExpanded ? 'w-64' : 'w-20'}`}>
      <div className="relative flex h-16 items-center justify-center border-b border-gray-800 p-4">
        {isExpanded && <span className="text-xl font-bold">Logistics Portal</span>}
...
export default LogisticsSideNav;