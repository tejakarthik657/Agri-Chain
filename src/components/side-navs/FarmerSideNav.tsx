...
const FarmerSideNav: React.FC<SideNavProps> = ({ isExpanded, toggleSidebar }) => {
  const navigate = useNavigate();
  const handleLogout = () => navigate('/'); // Navigate to landing page on logout

  const navItems = [
    { to: "dashboard", icon: <BsGrid1X2Fill />, name: "Dashboard" },
    { to: "add-batch", icon: <FaPlusCircle />, name: "Add New Batch" },
    { to: "my-batches", icon: <FaBoxOpen />, name: "My Batches" },
  ];
  ...
  const NavItem: React.FC<{ to: string, icon: React.ReactNode, name: string }> = ({ to, icon, name }) => (
  ...
  return (
    <aside className={`fixed left-0 top-0 z-20 flex h-screen flex-col bg-[#1c2536] text-white transition-all duration-300 ease-in-out ${isExpanded ? 'w-64' : 'w-20'}`}>
      <div className="relative flex h-16 items-center justify-center border-b border-gray-800 p-4">
        {isExpanded && <span className="text-xl font-bold">Farmer Portal</span>}
      ...
      <div className="flex flex-1 flex-col overflow-y-auto p-2">
        ...
        <div className="mt-auto border-t border-gray-700 pt-2">
          {bottomNavItems.map(item => <NavItem key={item.name} {...item} />)}
